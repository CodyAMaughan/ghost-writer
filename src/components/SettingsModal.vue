...
<template>
    <div v-if="isOpen" class="fixed inset-0 bg-black/90 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
        <div class="bg-slate-800 w-full max-w-md rounded-xl border border-slate-600 shadow-2xl overflow-hidden">
            ...
            <div class="p-6 space-y-8">
                ...
                <div class="space-y-2">
                    <div class="flex justify-between items-center">
                        ...
                        <button @click="toggleMute" class="text-slate-400 hover:text-white">
                            <VolumeX v-if="isMuted" class="w-5 h-5 text-red-500" />
                            <Volume2 v-else class="w-5 h-5" />
                        </button>
                    </div>
                    <input type="range" min="0" max="1" step="0.05" v-model.number="masterVolume" @input="update"
                           class="w-full accent-blue-500 h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer" 
                           :disabled="isMuted" :class="{ 'opacity-50': isMuted }" />
                </div>
                ...
                <div class="space-y-4 pt-4 border-t border-slate-700/50">
                    ...
                    <div class="space-y-2">
                         ...
                         <input type="range" min="0" max="1" step="0.05" v-model.number="musicVolume" @input="update"
                    ...
                    <div class="space-y-2">
                         ...
                         <div class="flex items-center gap-4">
                             <input type="range" min="0" max="1" step="0.05" v-model.number="sfxVolume" @input="update"
                                    class="w-full accent-green-500 h-1 bg-slate-700 rounded-lg appearance-none cursor-pointer" />
                             <button @click="testSfx" class="px-2 py-1 bg-slate-700 hover:bg-slate-600 rounded text-[10px] text-white">TEST</button>
                         </div>
                    </div>
                </div>
            ...
            <div class="p-4 bg-black/20 text-center">
                 <p class="text-[10px] text-slate-600">Audio engine powered by Howler.js</p>
            </div>
        </div>
    </div>
</template>

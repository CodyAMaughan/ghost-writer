    ...
    <div class="bg-slate-800 border-2 border-slate-600 rounded-xl w-full max-w-lg shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
      ...
      <div class="p-6 overflow-y-auto text-slate-300 space-y-6">
        <div class="bg-emerald-900/30 border border-emerald-500/50 p-4 rounded-lg">
          <h3 class="text-emerald-400 font-bold mb-1">
            RECOMMENDED: Google Gemini
          </h3>
          <p class="text-sm text-emerald-100/80">
            Fast, smart, and has a very generous free tier (no credit card usually required).
          </p>
        </div>
        ...
        <ol class="list-decimal list-inside space-y-4 text-sm marker:text-slate-500">
          <li class="pl-2">
            ...
            <div class="mt-2 flex items-center gap-2">
              ...
              <button
                class="p-2 bg-slate-700 hover:bg-slate-600 rounded text-white transition-colors"
                title="Copy Link"
                @click="copyLink"
              >
                <Check
              ...
              <a
                ...
                target="_blank"
                class="p-2 bg-purple-600 hover:bg-purple-500 rounded text-white transition-colors flex items-center gap-1"
              >
        ...
        <div class="border-t border-slate-700 pt-4 mt-4">
          <h3 class="text-white font-bold mb-2 text-sm">
          ...
          <a
            ...
            target="_blank"
            class="block w-full text-center p-2 bg-amber-900/30 text-amber-400 border border-amber-700/50 rounded hover:bg-amber-900/50 transition-colors text-xs font-bold"
          >
            GET ANTHROPIC KEY
          </a>
        </div>
      ...
      <div class="bg-slate-900 p-4 border-t border-slate-700 text-center">
        <button
          class="bg-white text-slate-900 hover:bg-slate-200 font-bold py-3 px-8 rounded transition-colors w-full"
          @click="$emit('close')"
        >
          Got it!
        </button>

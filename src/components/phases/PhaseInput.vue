...
<template>
    <div class="w-full flex-grow flex flex-col relative">
       ...
       <div v-if="!hasSubmitted" class="flex-grow grid grid-cols-1 md:grid-cols-2 gap-8 h-full">
          <!-- MANUAL -->
          <div :class="[theme.colors.card, theme.colors.border]" class="border bg-opacity-50 p-6 rounded-xl transition-all flex flex-col group hover:border-blue-500">
             <div class="flex items-center gap-3 mb-4" :class="theme.colors.accent">
                <Brain class="w-6 h-6" />
                <h3 class="font-bold tracking-widest">{{ theme.copy.manualInputLabel }}</h3>
             </div>
             ...
             <div class="flex justify-between mt-4">
                ...
                <button @click="submitManual" :disabled="!manualText" data-testid="submit-manual-btn" 
          ...
          <!-- GHOST -->
          <div @click="openGhostModal" data-testid="open-ghost-modal-btn" 
             ...
             <h3 class="font-bold tracking-widest text-xl group-hover:text-purple-400 text-center uppercase" :class="theme.colors.accent">
                 Use Ghost Writer<br><span class="text-xs normal-case opacity-70">(Auto-Generate)</span>
             </h3>
             <p class="text-xs text-slate-500 mt-2 uppercase">{{ theme.copy.ghostSubtext }}</p>
          </div>
       </div>

       <div v-else class="flex-grow flex flex-col items-center justify-center" data-testid="data-uploaded-msg">
          <CheckCircle class="w-20 h-20 mb-4 animate-bounce" :class="theme.colors.accent" />
          <h2 class="text-2xl font-bold text-white">{{ theme.copy.uploadComplete }}</h2>
          <p class="text-slate-400 mt-2">{{ theme.copy.waiting }}</p>
       </div>
       ...
       <div v-if="isGhostModalOpen" class="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
          <div class="bg-slate-900 border-2 border-purple-500 w-full max-w-3xl rounded-xl p-6 shadow-2xl relative max-h-[90vh] overflow-y-auto">
             ...
             <div v-if="!activeAgent" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                <div v-for="agent in ghostAgents" :key="agent.id" 
                ...
                <div @click="selectAgent({id:'custom'})"
                   ...
                   <div class="font-bold text-purple-400 uppercase tracking-wider text-xs">Custom Ghost</div>
                </div>
             </div>

...
<template>
  <div class="w-full flex-grow flex flex-col">
    ...
    <div class="text-center mb-6">
      <p class="text-slate-500 text-xs mb-1 uppercase tracking-widest">
        ORIGINAL PROMPT
      </p>
    ...
    <h2 class="text-center text-xl text-white font-bold mb-6">
      IDENTIFY THE HUMANS
    </h2>
       
    <div class="grid gap-4 flex-grow overflow-y-auto content-start pb-20">
      <div
        v-for="(sub, idx) in gameState.submissions"
        :key="idx" 
      ...
      >
        ...
        <div v-if="sub.authorId !== myId">
          <div
            v-if="!votesLocked"
            class="flex items-center gap-2 p-1 rounded-full border border-slate-700 bg-black/30"
          >
            <button
              ...
              @click="toggleVote(sub.authorId, 'HUMAN')"
            >
            ...
            <button
              ...
              class="px-4 py-2 rounded-full text-xs font-bold transition-all flex items-center gap-1"
              @click="toggleVote(sub.authorId, 'BOT')"
            >
          ...
          <div
            v-else
            class="px-4 py-2 bg-slate-900 rounded-full border border-slate-700 flex items-center gap-2"
          >
            <Lock class="w-3 h-3 text-slate-500" />
            <span
              class="text-xs font-bold"
              :class="votes[sub.authorId] === 'HUMAN' ? 'text-blue-400' : 'text-purple-400'"
            >
        ...
        <div
          v-else
          class="px-4 py-2 opacity-50 text-slate-500 text-sm border border-slate-700 rounded-full"
        >
          YOUR SUBMISSION
        </div>
      </div>
    ...
    <div
      v-else
      class="fixed bottom-8 right-8 text-right bg-slate-900/95 p-6 rounded-xl border border-slate-600 shadow-2xl animate-pulse flex flex-col items-end gap-2 z-50"
    >

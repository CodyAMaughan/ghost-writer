...
<template>
  <div class="w-full flex-grow flex flex-col items-center justify-center">
    ...
    <div class="w-full max-w-md bg-slate-800 border border-slate-700 rounded-xl overflow-hidden mb-8">
      <table class="w-full text-left">
        <thead class="bg-slate-900 border-b border-slate-700">
          <tr>
            <th class="p-4 text-xs text-slate-500">
              RANK
            </th>
            <th class="p-4 text-xs text-slate-500">
              OPERATIVE
            </th>
            <th class="p-4 text-xs text-slate-500 text-right">
              SCORE
            </th>
          </tr>
        </thead>
        <tbody>
          ...
          >
            ...
            <td
              ...
              :class="{'text-yellow-400': p.rank===1, [theme.colors.accent]: p.rank>1}"
            >
    ...
    >
      <button
        v-if="gameState.round < gameState.maxRounds"
        data-testid="next-round-btn"
        class="font-bold py-3 px-8 rounded text-white"
      ...
      >
        <button
          class="font-bold py-3 px-8 rounded text-white bg-blue-900 hover:bg-blue-800 border border-blue-700"
          @click="returnToLobby"
        >
          RETURN TO LOBBY
        </button>
        ...
        <button
          class="bg-red-900/50 hover:bg-red-900 text-red-200 border border-red-700 font-bold py-3 px-8 rounded"
          @click="leaveGame"
        >
          EXIT GAME
        </button>

...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur flex items-center justify-center p-4">
    <div class="bg-slate-800 border-2 border-green-500 rounded-xl w-full max-w-2xl shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
      <!-- Header -->
      <div class="bg-slate-900 p-4 border-b border-slate-700 flex justify-between items-center">
         <h2 class="text-xl font-bold text-green-400 tracking-widest uppercase">Mission Protocol</h2>
      ...
      <div class="p-6 overflow-y-auto space-y-6 text-slate-300">
         ...
         <section>
            <h3 class="text-white font-bold mb-2 flex items-center gap-2"><EyeOff class="w-4 h-4 text-purple-400"/> THE OBJECTIVE</h3>
            <p class="text-sm">
         ...
         <section>
            <h3 class="text-white font-bold mb-2 flex items-center gap-2"><Award class="w-4 h-4 text-yellow-400"/> SCORING MATRIX</h3>
         ...
         <section>
             <h3 class="text-white font-bold mb-2 flex items-center gap-2"><Eye class="w-4 h-4 text-blue-400"/> DETECTIVE BONUS</h3>
             <p class="text-sm">
      ...
      <div class="bg-slate-900 p-4 border-t border-slate-700 text-center">
          <button @click="$emit('close')" class="bg-green-500 text-black font-bold py-2 px-8 rounded hover:bg-green-400 transition-colors">ACKNOWLEDGE</button>
      </div>
    </div>
  </div>
</template>
